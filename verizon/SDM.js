var datamorphixConverseBot_width = 600;
var datamorphixConverseBot_height = 570;
//var datamorphixConverseBot_imgUrl = 'http://54.213.230.201/test/cross.png';

function datamorphixConverseBot_Init(showprompt, width, height, imgUrl) {

  if (width) datamorphixConverseBot_width = width;
  if (height) datamorphixConverseBot_height = height;
  if (imgUrl) datamorphixConverseBot_imgUrl = imgUrl;

  //datamorphixConverseBot_botId = bot;

  if (showprompt) {
    var chatprompt = document.createElement('div');
    chatprompt.className = 'chatprompt';
    chatprompt.onclick = datamorphixConverseBot_Open;
    chatprompt.innerHTML = '<img src="'+datamorphixConverseBot_imgUrl+'" width="80"/>';
    document.body.appendChild(chatprompt);
  }
}

function datamorphixConverseBot_Open() {

  Modal.open({
      content: '<iframe style="width:'+datamorphixConverseBot_width+'px; height:'+datamorphixConverseBot_height+'px; border:0" src="http://54.213.230.201/verizonUI/"></iframe>',
      draggable: false,
